...
import { onSnapshot, query, collection, orderBy, updateDoc, doc, deleteDoc, writeBatch, addDoc } from 'firebase/firestore';
  ...
  const handleAddProject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newProject.id) {
         ...
         // Logic to add to the TOP of the list:
         // 1. Find the current MIN order
         const currentOrders = projects.map(p => p.order !== undefined ? p.order : 9999);
         const minOrder = currentOrders.length > 0 ? Math.min(...currentOrders) : 0;
         
         // 2. Assign new order as minOrder - 1
      ...
      setNewProject({ questions: [], ownershipType: 'own' });
      setFileToUpload(null);
    } catch (error: any) {
                       ...
                       <div className="border-t border-b border-gray-50 py-2">
                          {isApp ? (
                              ...
                              <div className="text-sm">
                                ...
                                <span className="font-semibold text-gray-700">{sub.department || 'Загальне питання'}</span>
                              </div>
                          )}
                       </div>
             ...
             {isAddingProject && (
              <div className="bg-white p-6 rounded-2xl shadow-lg border border-kmmr-pink/20 mb-6 animate-fade-in-up">
                 <div className="flex justify-between items-start mb-6">
                   ...
                   <button onClick={() => { setIsAddingProject(false); setNewProject({ questions: [], ownershipType: 'own' }); setFileToUpload(null); }} className="text-gray-400 hover:text-gray-600"><XCircle size={24}/></button>
                </div>
                <form onSubmit={handleAddProject} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     ...
                     <div className="space-y-4">
                        ...
                        <div>
                           <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Тип проєкту</label>
                           <select 
                             className="border p-2 rounded w-full" 
                             value={newProject.ownershipType || 'own'} 
                             onChange={e => setNewProject({...newProject, ownershipType: e.target.value as any})}
                           >
                              <option value="own">Наш проєкт</option>
                              <option value="support">За нашої підтримки</option>
                              <option value="partner">Інша організація</option>
                           </select>
                        </div>
                        {newProject.ownershipType === 'partner' && (
                           <div>
                              <label className="text-xs font-bold text-gray-500 uppercase block mb-1">Назва організації</label>
                              <input 
                                className="border p-2 rounded w-full" 
                                value={newProject.partnerName || ''} 
                                onChange={e => setNewProject({...newProject, partnerName: e.target.value})} 
                  ...
                  <div className="flex gap-2 pt-4 border-t border-gray-100">
                    ...
                    <button type="button" onClick={() => { setIsAddingProject(false); setNewProject({ questions: [], ownershipType: 'own' }); setFileToUpload(null); }} className="bg-gray-100 text-gray-700 px-6 py-3 rounded-lg font-bold">Скасувати</button>
                  </div>
                </form>
              </div>
             )}
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
               <div onClick={() => { setNewProject({ questions: [], ownershipType: 'own' }); setFileToUpload(null); setIsAddingProject(true); }} className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-2xl flex flex-col items-center justify-center h-96 cursor-pointer hover:bg-gray-100 transition-colors group">
